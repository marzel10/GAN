classdef SharedtranspConv < nnet.layer.Layer ...
    ...
    methods
        function layer = SharedtranspConv(name, numStreams, kernel, in_chan, out_chan, stride)
            arguments
                name (1,1) string
                numStreams (1,1) double {mustBeInteger,mustBePositive} = 28
            ...
            % layer.InputNames  = arrayfun(@(i) sprintf("in%d",i), 1:numStreams, 'UniformOutput', false);
            % layer.OutputNames = arrayfun(@(i) sprintf("out%d",i), 1:numStreams, 'UniformOutput', false);
            ...
            layer.W1 = SharedtranspConv.initXavier([k1(1) k1(2) inC  out1]);
        ...
        function varargout = predict(layer, varargin)
            ...
            for i = 1:n
                X = varargin{i};  % BSSC
                Y = dltranspconv(X, layer.W1, layer.B1, ...
                           'Stride', layer.Stride1, 'Cropping','same');
