classdef net_builder
    ...
    methods(Static)
        ...
        function layer = helperReshape(name, output_size, batch_size)
            % Custom reshape layer using functionLayer
            layer = custom_reshape(name, output_size, batch_size);
        end
        ...
        function net = build_net_with_28_inputs(latent_size)
            ...
            kernel_conv1 = [3 2];  % Slightly larger for better pattern capture
            channels_conv1 = 16;   % More channels for richer features
            kernel_pool1 = [2 1];  % Gentler pooling to preserve temporal info
            stride_pool1 = [2 1];

            dropout_rate = 0.2;    % Much lower dropout rate

            kernel_conv2 = [3 2];
            channels_conv2 = 8;    % Reduced for better gradient flow
            kernel_pool2 = [2 1];  % Gentler pooling
            stride_pool2 = [2 1];

            single_input_size = 1000 * 2 * 8;  % Size of one flattened input
            for i=1:num_in
            ...
            hidden_size = 2000;    % Increased to handle larger input
            ...
            for i=1:num_in
                tempNet = [
                    ...
                    net_builder.helperchange_format(sprintf("reshape_%d",i),"BSSC",[1000,2,8])
