...
modelFile = 'C:\Users\Maria\Documents\Honours Programme\Networks\GAN\results\f4_GAN_Net_wls_with_AE\fc_all_path_net_loss_0.203931.mat';
modelFile = 'C:\Users\Maria\Documents\Honours Programme\Networks\GAN\results\f4_GAN_Net_wls_with_AE\fc_all_path_net_loss_0.060286.mat';
modelFile_conv_relu = 'C:\Users\Maria\Documents\Honours Programme\Networks\GAN\results\f4_Convolutional_Path_Net_with_AE\Convolutional_Path_Net_loss_0.691875.mat';
modelFile_conv_square = 'C:\Users\Maria\Documents\Honours Programme\Networks\GAN\results\f4_Convolutional_Path_Net_with_AE\Convolutional_Path_Net_loss_square_1.051842.mat'
modelFile_GAN_relu = 'C:\Users\Maria\Documents\Honours Programme\Networks\GAN\results\f4_GAN_Net_with_AE\f4_GAN_Net_loss_92.972649.mat';
modelFile_GAN_square = 'C:\Users\Maria\Documents\Honours Programme\Networks\GAN\results\f4_GAN_Net_with_AE\GAN_Net_loss_square_1.306862.mat';

modelFile= 'C:\Users\Maria\Documents\Honours Programme\Networks\GAN\results\f4_GAN_Net_with_AE\GAN_Net_loss_0.941966.mat'; % choose which model to use
...
if single_freq
    if GAN_mode
        ...
        if convolutional_mode
            % Convolutional GAN mode: spatial latent maps, SSCB input format
            rawDs = latent_space_datastore(panel_number, miniBatchSize, overlap, frequencies, true, AE_net_folder, ...
            'cacheFile', cf, 'convolutional_mode', true);
        else
            rawDs = latent_space_datastore(panel_number, miniBatchSize, overlap, frequencies, true, AE_net_folder, ...
            'cacheFile', cf, 'convolutional_mode', false);
        end
    else
...
for i = 1:rawDs.NumObservations
    ...
    if w == (nInputs + 1)
        ...
        for k = 1:nInputs
            ...
            if GAN_mode
                if convolutional_mode
                    inputCells{k} = coerceToDlInput(raw, true, 'SSCB');
                else
    ...
    elseif w == 2
        ...
        if nInputs == 1
            if GAN_mode
                if convolutional_mode
                    % Convolutional GAN expects SSCB 
                    inputCells{1} = coerceToDlInput(raw, true, 'SSCB');   
                else
            ...
            if ndims(raw) == 2
                ...
                for k = 1:min(nInputs,C)
                    if GAN_mode
                        if convolutional_mode
                            inputCells{k} = coerceToDlInput(raw(:,k), true, 'SSCB'); % S x 1 -> SSCB
                        else
                ...
                for k = 1:min(nInputs,C)
                    if GAN_mode
                        if convolutional_mode
                            ...
                            inputCells{k} = coerceToDlInput(raw(:,:,k), false, 'SCB'); % S x S2 -> SCB
                        end
                    else
...
for s = state_to_plot
    sHI_path = latent_space(paths_to_plot, s)';
