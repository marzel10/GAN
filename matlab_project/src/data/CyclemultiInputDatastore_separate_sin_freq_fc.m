classdef CyclemultiInputDatastore_separate_sin_freq_fc< matlab.io.Datastore & ...
    ...
    methods
        function ds = CyclemultiInputDatastore_separate_sin_freq_fc(cycleStructArray, numInputs, miniBatchSize, overlap_size, numCycles, envelope, frequency, varargin)
                    ...
                    if i <= length(allPairData)
                        cycleData = allPairData{i};
                            ...
                            if batchCount == 0 && i == 1 && isempty(allData{1})
                                ...
                                for j = 1:ds.NumInputs
                                    allData{ds.NumInputs+j} = zeros(B, numFeatures, 'like', cycleData);
                                end
                            end

                            % Fill this batch row (flatten each sample)
                            allData{i}(batchCount + 1, :) = cycleData(:)';  % input i -> [1x800]
                            allData{i + ds.NumInputs }(batchCount + 1, :) = cycleData(:)'; % target i -> [1x800]
