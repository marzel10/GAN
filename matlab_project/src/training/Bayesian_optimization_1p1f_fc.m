...
for p_i= 2:28
    ...
    k_sparse = optimizableVariable('k_sparse', [5, 15], 'Type', 'integer');
    hl_s1 = optimizableVariable('hidden_layer_size1', [1000, 2000], 'Type', 'integer');
    hl_s2 = optimizableVariable('hidden_layer_size2', [500, 1000], 'Type', 'integer');
    hl_s3 = optimizableVariable('hidden_layer_size3', [100, 500], 'Type', 'integer');
    hl_s4 = optimizableVariable('hidden_layer_size4', [30, 100], 'Type', 'integer');
    drop_rate = optimizableVariable('drop_rate', [1, 7], 'Type', 'integer');  % Representing 0.1 to 0.7
    ...
    optimVars = [k_sparse, hl_s1, hl_s2, hl_s3, hl_s4, drop_rate]; %, batch_size, learning_rate, lr_drop_factor, lr_drop_period];  % Use compressed version to enforce latent size constraint
    network_type = 'Sparse_Fully_Connected_AE'; 
    ...
    row = table(dataCfg.frequency, dataCfg.path_idx, bestParams.k_sparse, bestParams.hidden_layer_size1, bestParams.hidden_layer_size2, bestParams.hidden_layer_size3, bestParams.hidden_layer_size4, bestParams.drop_rate, ...
        'VariableNames', {'frequency', 'path_idx', 'sparse_size', 'hl_s1', 'hl_s2', 'hl_s3', 'hl_s4', 'drop_rate'});
    if isfile(latentCsv)
...
function loss = trainAndEvaluateAE(params, dataCfg, varargin)
    ...
    try        
        ...
        params.drop_rate = params.drop_rate / 10.0;  % Convert back to [0.1, 0.7]
        ...
        lossFn = @(varargin) k_sparse_MSE_loss(params.k_sparse,varargin{:});
        % Train network
        ...
        model_name = sprintf('%dp%df_k_%d_h1_%d_h2_%d_h3_%d_h4_%d_dr_%d_bs_%d_valLoss_%f.mat', ...
                dataCfg.path_idx, dataCfg.frequency, params.k_sparse, params.hidden_layer_size1, params.hidden_layer_size2, params.hidden_layer_size3, params.hidden_layer_size4, ...
...
function L = k_sparse_MSE_loss(k,varargin)
    numOut = numel(varargin)/2;
    accum = 0;
    for k=1:numOut
        Yk = varargin{1,k};
