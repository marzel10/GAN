...
import numpy as np
from states_check import states
...
class Panel_GraphDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        print(f"Loaded raw features with shape: {features.shape}")
        ...
        for state in range(features.shape[1]):
            ...
            if (x == 0).all():
                print(f"Warning: All features are zero for state {state}. Check if the raw features are correct.")
            ...
            if (adj_matrix == 0).all():
                print(f"Warning: Adjacency matrix for state {state} is all zeros. Check if the attention values are correct.")
            
            
            adj_matrix = adj_matrix[connection_matrix==1]  # shape: num_edges
            edge_weight = torch.tensor((np.array(adj_matrix)), dtype=torch.long)
