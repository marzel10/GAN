...
over the per-frequency model_database.xlsx files that BO_features.py's
...
def load_all_databases(folders=FOLDERS, root=PROJECT_ROOT):
    ...
    each row with its source folder. Folders that don't exist yet (that frequency's
    sweep hasn't been run) are skipped, not an error.'''
    frames = []
...
def plot_hp_vs_path(df, out_dir=OUT_DIR):
    ...
    for col, title in HP_PLOTS:
        ...
        for i, freq in enumerate(freqs):
            ...
            if freq_rows.empty:
                continue
            ax.plot(freq_rows["path_index"], freq_rows[col], "o",
                    color=cmap(i % 10), label=f"freq {int(freq)}")
            any_series = True
        if not any_series:
            plt.close(fig)
            continue
        ...
        ax.grid(True)
        fig.tight_layout()
        save_path = out_dir / f"AE_hp_summary_{col}.png"
