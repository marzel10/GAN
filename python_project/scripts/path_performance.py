...
Lastly 4 plots are created for each metric, where the x axis is the path number and the y
axis is the metric value, lines are the frequencies, metrics is taken for the ensemble model.

Some path folders aren't trained yet (BO runs still in progress) -- those are left as
None/NaN and simply produce a gap in the corresponding line.
'''
...
from config import BASE_PANELS, FREQUENCY_MAPPING


BO_folders = [f"Multi_path_BO_fixed_freq{i}" for i in range(6)]
PANELS = BASE_PANELS + ["123"]           # 103, 104, 105, 109, 123 -- panel axis (size 5)
FOLD_KEYS = BASE_PANELS + ["ensemble"]   # Model_val_<panel>.keras x4 + ensemble_model.keras -- fold axis (size 5)
N_PATHS = 28
...
def average_sHI_over_frequency(sHI, panel, fold="ensemble"):
    ...
    (missing frequency/path combos are skipped, not zero-filled). Returns a length-N_PATHS
    list of 1-D arrays (state order); entries are None where every frequency is missing.'''
...
def plot_metrics(metrics, out_dir=OUT_DIR):
    ...
    for metric_idx, metric_name in enumerate(METRIC_NAMES):
        ...
        ax.grid(True)
        fig.tight_layout()
        fig.savefig(out_dir / f"{metric_name}_vs_path.png")
        plt.close(fig)
        print(f"Saved: {out_dir / f'{metric_name}_vs_path.png'}")

KEY_TO_TITLES = {"103": "L1-03", "104": "L1-04", "105": "L1-05", "109": "L1-09", "123": "L1-23"}

def plot_sHI_grid(sHI, out_dir=OUT_DIR):
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    freq_labels = [f"{FREQUENCY_MAPPING[i]} kHz" for i in range(len(BO_folders))] + ["average"]
    ...
    for freq_idx in range(len(BO_folders)):
        for fold_idx, fold_key in enumerate(FOLD_KEYS):
            ...
            for panel_idx, panel in enumerate(PANELS):
                ...
                ax.plot(life_fraction, mean_curve, label=panel)
                per_freq_means[fold_idx][panel_idx].append(mean_curve)
                any_data = True
            if not any_data:
    ...
    for fold_idx, fold_key in enumerate(FOLD_KEYS):
        ...
        for panel_idx, panel in enumerate(PANELS):
            if not per_freq_means[fold_idx][panel_idx]:
                continue
            overall_mean = np.mean(np.stack(per_freq_means[fold_idx][panel_idx], axis=0), axis=0)
            life_fraction = np.linspace(0, 1, overall_mean.shape[0])
            ax.plot(life_fraction, overall_mean, label=KEY_TO_TITLES.get(panel, panel))
            any_data = True
        if not any_data:
            ax.text(0.5, 0.5, "no data", ha="center", va="center", transform=ax.transAxes, fontsize=7)
        if fold_idx == 0:
    ...
    fig.tight_layout()
    save_path = out_dir / "sHI_grid.png"
...
def load_cached(out_dir=OUT_DIR):
    '''Loads a previously saved (sHI, metrics) pair from out_dir, or returns None if either
    file is missing (e.g. first run).'''
    sHI_path = out_dir / "sHI.pkl"
    metrics_path = out_dir / "metrics.npy"
    if not sHI_path.exists() or not metrics_path.exists():
        return None
...
def main(recompute=False):
    ...
    if cached is not None:
        ...
        sHI, metrics = compute_sHI_and_metrics()
