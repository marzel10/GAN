classdef CyclemultiInputDatastore_separate < matlab.io.Datastore & ...
    ...
    methods
        function ds = CyclemultiInputDatastore_separate(cycleStructArray, numInputs, miniBatchSize, overlap_size, numCycles, envelope)
                ...
                for i = 1:ds.NumInputs
                    if i <= length(allPairData)
                        ...
                        if batchCount == 0 && i == 1 && isempty(allData{1})
                            ...
                            for j = 1:ds.NumInputs
                                allData{ds.NumInputs+j} = zeros(B, T, S, C, 'like', cycleData);
                            end
                        end
